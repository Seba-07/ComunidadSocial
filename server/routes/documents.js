...
function getValidationUrl() {
  ...
  if (!url) {
    ...
    if (vercelUrl) return `https://${vercelUrl}/app/validar`;
    const isDeployed = process.env.NODE_ENV === 'production' || !!process.env.RAILWAY_ENVIRONMENT;
    if (isDeployed) return null; // Return null instead of throwing — QR will be skipped
...
async function registerDocumentAndGetQR({ organizationId, documentType, documentTitle, issuedBy, assemblyId, pdfBuffer }) {
  ...
  const baseUrl = getValidationUrl();

  // If no validation URL available (missing FRONTEND_URL in production), skip QR
  if (!baseUrl) {
    return { folio, qrDataUrl: null, verifyUrl: null, record };
  }

  const verifyUrl = `${baseUrl}/${folio}`;
