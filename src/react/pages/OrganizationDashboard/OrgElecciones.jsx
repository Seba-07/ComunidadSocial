...
export default function OrgElecciones({ org, onRefresh }) {
  ...
  async function loadElectionStatus() {
    ...
    try {
      ...
      // Fallback to org data if endpoint not deployed yet
      setElectionData({
  ...
  return (
    <div>
      ...
      {/* Status Card */}
      <div style={{
        background: config.bg, border: `2px solid ${config.border}`, borderRadius: 12,
        ...
        <div className="r-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
          <div style={{ background: 'white', borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: '#111827' }}>
              {formatDate(electionData?.boardElectionDate)}
            </div>
          </div>
          <div style={{ background: 'white', borderRadius: 8, padding: 12 }}>
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>Vencimiento del Mandato</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: '#111827' }}>
              {formatDate(electionData?.boardExpirationDate)}
            </div>
            {expirationDays !== null && (
              <div style={{
                ...
                {expirationDays <= 0
                  ? `Vencido hace ${Math.abs(expirationDays)} días`
                  : expirationDays <= 30
                    ? `Vence en ${expirationDays} días`
      ...
      {/* TRICEL Document info (when in electoral process) */}
      {status === 'EN_PROCESO_ELECTORAL' && electionData?.tricelDocument && (
        <div style={{
          ...
          <div style={{ display: 'grid', gap: 8, fontSize: 14 }}>
            {electionData.tricelDocument.plannedElectionDate && (
              <div>
                <span style={{ color: '#6b7280' }}>Fecha planificada de elección: </span>
                <span style={{ fontWeight: 600, color: '#111827' }}>{formatDate(electionData.tricelDocument.plannedElectionDate)}</span>
              </div>
            )}
            {electionData.tricelDocument.fileName && (
              <div>
                <span style={{ color: '#6b7280' }}>Acta TRICEL: </span>
                <span style={{ fontWeight: 500, color: '#111827' }}>{electionData.tricelDocument.fileName}</span>
              </div>
            )}
            {electionData.tricelDocument.uploadedAt && (
              <div>
                <span style={{ color: '#6b7280' }}>Registrado: </span>
                <span style={{ color: '#111827' }}>{formatDate(electionData.tricelDocument.uploadedAt)}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <button
          ...
          onClick={() => setShowTricelModal(true)}
          style={{
            padding: '14px 28px', border: 'none', borderRadius: 10,
            background: canStartElection ? '#1e40af' : '#d1d5db',
            color: canStartElection ? '#fff' : '#9ca3af',
            fontSize: 15, fontWeight: 700, cursor: canStartElection ? 'pointer' : 'not-allowed',
            display: 'flex', alignItems: 'center', gap: 8
          }}
        >
          <span>🗳️</span> Informar Comisión Electoral (TRICEL)
        </button>
        {!canStartElection && status === 'EN_PROCESO_ELECTORAL' && (
          <p style={{ fontSize: 13, color: '#d97706', marginTop: 8 }}>
            Ya existe un proceso electoral en curso. Espere a que finalice antes de iniciar otro.
          </p>
        )}
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: 24 }}>
        ...
        {cargos.length > 0 ? (
          ...
          <p style={{ color: '#6b7280', textAlign: 'center', padding: 16 }}>
            No hay información de directorio disponible.
          </p>
        )}
      </div>
      ...
      {showTricelModal && (
        <TricelModal
          orgId={org._id}
          onClose={() => setShowTricelModal(false)}
          onSuccess={() => {
            setShowTricelModal(false);
            loadElectionStatus();
...
function TricelModal({ orgId, onClose, onSuccess, addToast }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!plannedDate) {
      addToast('Debe indicar la fecha planificada para la elección', 'error');
      return;
    }
    ...
    try {
      ...
      if (file) {
        formData.append('tricelDocument', file);
      }
      ...
      const response = await fetch(`${apiService.baseUrl}/organizations/${orgId}/elections/start`, {
        ...
        headers: {
          ...
          ...(apiService._authToken && { Authorization: `Bearer ${apiService._authToken}` })
        }
      });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.error || 'Error al iniciar proceso electoral');
      }
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 520, overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb' }}>
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#111827' }}>
            Informar Comisión Electoral (TRICEL)
          </h3>
          <p style={{ margin: '8px 0 0', fontSize: 13, color: '#6b7280' }}>
            Al confirmar, el estado de la directiva cambiará a "En Proceso Electoral".
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 24 }}>
          ...
          <div style={{
            ...
            padding: 14, marginBottom: 20, fontSize: 13, color: '#1e40af', lineHeight: 1.5
          }}>
            Según la Ley 19.418, la Comisión Electoral (TRICEL) es responsable de organizar
            y supervisar el proceso de elección del nuevo directorio. Adjunte el acta de
            designación del TRICEL si está disponible.
          </div>

          {/* Planned election date */}
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: '#374151', marginBottom: 8 }}>
              Fecha planificada de elección *
            </label>
            <input
              type="date"
              value={plannedDate}
            ...
            {file ? (
              <div style={{
                ...
                <button type="button" onClick={() => { setFile(null); if (fileRef.current) fileRef.current.value = ''; }}
                  style={{ padding: '2px 10px', border: '1px solid #ef4444', borderRadius: 6, background: 'white', color: '#ef4444', fontSize: 12, cursor: 'pointer' }}>
                  Quitar
                </button>
              </div>
            ) : (
              <label style={{
                display: 'block', padding: '16px', border: '2px dashed #d1d5db',
                borderRadius: 10, textAlign: 'center', cursor: 'pointer', color: '#6b7280', fontSize: 14
              }}>
                ...
                <input ref={fileRef} type="file" accept=".pdf" style={{ display: 'none' }}
                  onChange={(e) => {
                    ...
                    if (f && f.size > 20 * 1024 * 1024) {
                      addToast('El archivo no puede superar 20MB', 'error');
                      return;
                    }
                    setFile(f);
                  }}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button type="submit" disabled={submitting}
              style={{
                flex: 1, padding: 14, background: '#1e40af', border: 'none', borderRadius: 10,
